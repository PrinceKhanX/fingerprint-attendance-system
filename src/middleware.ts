...
const protectedRoutes = ['/admin', '/teacher', '/student', '/guardian', '/analytics']
...
export async function middleware(request: NextRequest) {
  ...
  // Analytics page is accessible by all authenticated users
  if (pathname.startsWith('/analytics')) {
    const token = request.cookies.get('auth_token')?.value || request.cookies.get('guardian_auth_token')?.value
    if (!token) {
